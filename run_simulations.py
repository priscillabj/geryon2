...
def prepare_source(filename):
    ...
    print(f"[timing] drw_fit {time.time()-t:.1f}s", flush=True)
...
def run_slice(prep):
    ...
    print(f"[rank {rank}] gpSim {time.time()-t:.1f}s slice={slice_size}", flush=True)
    ...
    print(f"[rank {rank}] SF+linmix {time.time()-source_start:.1f}s for {slice_size} sims", flush=True)

    # ── save LC parquets if requested ─────────────────────────────────────────
    t = time.time()
    if SAVE_LC:
        save_lc_parquet(ra, dec, band, rows_daily, rows_orgcad)
    print(f"[rank {rank}] save_lc {time.time()-t:.1f}s", flush=True)
...
def main():
    ...
    for f in todo:
        ...
        if rank == 0:
            print(f"[rank 0] gather {time.time()-t:.1f}s", flush=True)
            try:
                t = time.time()
                merge_and_save(prep["ra"], prep["dec"], prep["band"],
                               prep["time_arr"], prep["dur"], all_slices)
                print(f"[rank 0] merge_and_save {time.time()-t:.1f}s", flush=True)
